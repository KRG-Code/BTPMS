...
import ResourcesAuditReport from './ResourcesAuditReport';
import PDFPasswordModal from '../PersonelsComponents/PDFPasswordModal';
...
const ResourcesReportModal = ({ isOpen, onClose, isDarkMode }) => {
  ...
  const [auditCategory, setAuditCategory] = useState('equipment'); // 'equipment' or 'vehicle'

  useEffect(() => {
    if (isOpen) {
      fetchReportData();
    }
  }, [isOpen, reportType, startDate, endDate, auditCategory]);
  ...
  const handleConfirmDownload = async (password) => {
    ...
    try {
      // Create filename with audit category
      const fileName = `${auditCategory}_audit_${reportType}_${format(new Date(), 'yyyy-MM-dd')}`;
      ...
      >
        <div className={`p-4 border-b ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-gray-50'} flex justify-between items-center`}>
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-semibold">Resources Audit Report</h2>
          </div>
        </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div>
                <label className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  <FaFilter className="inline mr-1" /> Report Category
                </label>
                <select
                  value={auditCategory}
...
export default ResourcesReportModal;
